...
Permanently disabling a query opts out of many great features that BlazorQuery has to offer (like background refetches), and it's also not the idiomatic way. It takes you from the **declarative approach** (defining dependencies when your query should run) into an **imperative mode** (fetch whenever I click here).
...
- BlazorQuery: Class-based, manual execution
- Both: Same `enabled` option behavior

## Note on skipToken

React Query provides `skipToken` as a type-safe alternative to `enabled: false`. BlazorQuery doesn't have an equivalent because:
