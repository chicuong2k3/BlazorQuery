...
If the number of queries you need to execute is changing dynamically, you cannot manually create query instances. Instead, BlazorQuery provides a `UseQueries<T>` class, which you can use to dynamically execute as many queries in parallel as you'd like.
...
**Key Differences**:
- BlazorQuery requires explicit `ExecuteAllAsync()` call
- BlazorQuery uses class instances instead of hooks
