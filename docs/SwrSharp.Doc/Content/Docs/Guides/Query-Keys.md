...
This is because:
- Property order in anonymous or record types doesn't matter (properties are sorted by name before comparison).
- Null properties are ignored during comparison.
...
> **Mathematical analogy**: This is similar to how **tuples** work in mathematics.
> A tuple is an ordered sequence where position matters: `(a, b) ≠ (b, a)` unless `a = b`.
> Similarly, `QueryKey("todos", "active", 2)` and `QueryKey("todos", 2, "active")` are distinct keys
> because the position of each element is part of its identity.

```csharp
// These are NOT equal (like tuples: (todos, active, 2) ≠ (todos, 2, active))
...
> If changing a variable would require fetching different data,
