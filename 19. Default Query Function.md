...
If you find yourself wishing that you could just share the same query function for your entire app and just use query keys to identify what it should fetch, you can do that by providing **type-safe default query functions** to SwrSharp!

## Type-Safe Design

Unlike JavaScript libraries, SwrSharp uses a **type-safe approach** where you register default functions per type. This eliminates runtime casting and provides compile-time type safety.
...
- ✅ **No runtime casting** - types are validated at compile time
- ✅ **Better performance** - no boxing/unboxing or type checking at runtime
- ✅ **Type-specific logic** - different types can have different default behaviors
- ✅ **Clearer error messages** - know exactly which type is missing a default
...
```csharp
// ✅ Good: Simple REST API with consistent endpoints
var queryClient = new QueryClient
{
    DefaultQueryFn = async ctx => {
        var endpoint = ctx.QueryKey.Parts[0]?.ToString();
...
```csharp
// ✅ Good: Use default for most queries, override when needed
...
1. **No Runtime Casting**: SwrSharp's type-safe approach eliminates the need for runtime type checking and casting, improving performance.
...
- ✅ **Type-Safe Design**: Register per-type default functions with `SetDefaultQueryFn<T>()`
- ✅ **No Runtime Casting**: Compile-time type safety eliminates runtime errors
- ✅ **Query key determines fetch**: Different endpoints, same type, same function
- ✅ **No repetitive queryFn definitions**: Set once per type, use everywhere
- ✅ **Override for special cases**: Provide custom queryFn when needed
- ✅ **Better than React Query**: Compile-time safety vs runtime checks
- ✅ **Supports CancellationToken**: Full async cancellation support
- ✅ **Perfect for REST APIs**: Consistent typed endpoints
- ✅ **C# Idiomatic**: Leverages C#'s strong type system

**Use type-safe default query functions for clean, type-safe queries!** 🎯
