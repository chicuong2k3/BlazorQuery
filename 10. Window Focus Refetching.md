...
If a user leaves your application and returns and the query data is stale, **BlazorQuery automatically requests fresh data for you in the background**. You can disable this globally or per-query using the `refetchOnWindowFocus` option.
...
4. BlazorQuery automatically refetches stale queries in the background
...
BlazorQuery provides an abstraction (`IFocusManager`) that allows different platforms to implement focus detection their own way.
