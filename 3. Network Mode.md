...
when there is no network connection. You can set the network mode for each query/mutation individually 
or globally via `QueryClient.DefaultNetworkMode`.
...
> You can observe network state changes via `OnlineManager.IsOnline` to reactively control 
query execution.
...
| Network Mode     | Network             | FetchStatus     | IsLoading                   | IsFetching | IsPaused | IsError | IsSuccess            |
| ---------------- | ------------------- | --------------- | --------------------------- | ---------- | -------- | ------- | -------------------- |
| **Online**       | Online              | Fetching → Idle | true (while fetching)       | true       | false    | false   | true (after success) |
|                  | Offline             | Paused          | true if Data == null        | false      | true     | false   | false if no data     |
| **Always**       | Online              | Fetching → Idle | true (while fetching)       | true       | false    | false   | true (after success) |
|                  | Offline             | Fetching → Idle | true (while fetching fails) | true       | false    | true    | false                |
| **OfflineFirst** | Online              | Fetching → Idle | true (while fetching)       | true       | false    | false   | true (after success) |
|                  | Offline, Data stale | Paused          | true                        | false      | true     | false   | false                |
|                  | Offline, Data fresh | Idle            | false                       | false      | false    | false   | true                 |
