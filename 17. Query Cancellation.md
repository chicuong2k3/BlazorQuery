...
You might want to cancel a query manually. For example, if the request takes a long time to finish, you can allow the user to click a cancel button to stop the request. To do this, you just need to call `queryClient.CancelQueries(filters)`, which will cancel the query and revert it back to its previous state. If you have consumed the `Signal` (CancellationToken) in the query function, BlazorQuery will additionally also cancel the Task.
...
```csharp
// ✅ Good: Let BlazorQuery handle cancellation
queryFn: async ctx => {
    var response = await httpClient.GetAsync("/api/data", ctx.Signal);
    return await response.Content.ReadFromJsonAsync<Data>(
        cancellationToken: ctx.Signal
