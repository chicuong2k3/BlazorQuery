...
SwrSharp can retrieve cached data synchronously:
...
```
User Journey:
1. Load posts list (fetch: 1s) → scroll to position 500px
2. Click post #5 → navigate to detail
3. Read post
4. Click back
...
// SwrSharp caching ensures data is available immediately
...
// But SwrSharp caching still ensures data is available
...
```csharp
// ✅ Good: Cache data long enough for navigation
new QueryOptions<List<Post>>(
    queryKey: new("posts"),
    queryFn: async ctx => await FetchPostsAsync(),
...
```csharp
// ✅ Good: Use placeholder for paginated lists
new QueryOptions<PostsPage>(
...
```csharp
// ✅ Good: Prefetch next page for instant navigation
...
```csharp
// ✅ Good: Invalidate on mutations to keep data fresh
...
- ✅ Scroll restoration **Just Works™️** with SwrSharp
- ✅ No special configuration needed
- ✅ Works because of smart caching (5-minute default)
- ✅ Data available synchronously on return navigation
- ✅ No layout shifts = reliable scroll position
- ✅ Background refetch updates data without disrupting scroll
- ✅ Works with all query types (regular, paginated, infinite)
- ✅ Use `placeholderData` for even smoother transitions
- ✅ Browser/framework handles actual scroll restoration
- ✅ SwrSharp ensures stable layout for restoration to work

**The secret: Cached data + stable layout = perfect scroll restoration!** 📜✨
