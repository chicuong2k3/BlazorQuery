...
SwrSharp applies several optimizations to ensure that your components only re-render when they actually need to. While some optimizations differ from React Query due to C#/.NET architecture, the core principles remain the same.

## Structural Sharing

SwrSharp uses a technique called "structural sharing" to ensure that as many references as possible will be kept intact between re-renders. If data is fetched over the network, usually, you'll get a completely new reference by JSON parsing the response. However, SwrSharp will keep the original reference if _nothing_ changed in the data.

> **Note**: This optimization works best with immutable data types. For reference types, SwrSharp uses reference equality checks to determine if data has changed.
...
SwrSharp uses an event-based system (`OnChange` event) to notify components when query state changes. Components only re-render when they explicitly subscribe to these events:
...
SwrSharp's `UseQuery` only fires `OnChange` when meaningful state changes occur:
...
SwrSharp uses reference equality (`ReferenceEquals`) for objects:
...
**SwrSharp optimizations ensure your Blazor apps stay fast and responsive!** ⚡
